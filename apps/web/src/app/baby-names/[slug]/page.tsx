...
import { getAllBabyNames, getBabyNameBySlug, getBabyNameById, getBabyNamesByCategory } from "@/lib/baby-names/store";
import { getEntity } from "@/lib/knowledge/store";
import { babyNameSeo, babyNameCategorySeo } from "@/lib/seo/titles";
...
export async function generateStaticParams() {
  ...
  const categoryParams = VALID_CATEGORIES.map((c) => ({ slug: c }));
  const nameParams = names.map((n) => ({ slug: n.slug }));
  return [...categoryParams, ...nameParams];
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  if (isValidCategory(slug)) {
    return buildPageMetadata(babyNameCategorySeo(slug));
  }
...
export default async function BabyNamePage({ params }: PageProps) {
  const { slug } = await params;
